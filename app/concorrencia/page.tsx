    ...
    const handlePointClick = (payload: any) => {
        ...
        const items = Object.keys(payload)
            ...
            .map(k => {
                ...
                // Try to find by id_numerica FIRST (since stats uses it as key preference)
                // If not found, try by internal ID
                const found = competitors.find(c =>
                    (c.id_numerica && c.id_numerica.toString() === id.toString()) ||
                    c.id.toString() === id.toString()
                );

                return {
                    id,
                    name: found?.nome_anuncio || `Anúncio ${id}`,
                    price: payload[k],
                    fullData: found ? { airbnb_data: found, ...found } : null
                }
            })
            .sort((a, b) => b.price - a.price);

        // History logic: Since we don't have historical extractions here in this specific view (aggregates mainly), 
        // we can attempt to just show the current point or empty history. 
        // If we want history, we would need to fetch it or restructure the data.
        // For now, let's leave history empty or simulate it if we had data.
        const history: any[] = [];
