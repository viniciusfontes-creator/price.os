...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const valorFinal =
    parsed.data.acao === "aprovar"
      ? parsed.data.valor_editado ?? Number(current.meta_sugerida)
      : null
  ...
  const { data: updated, error: updErr } = await supabase
    ...
    .update({
      ...
      meta_aplicada: novoStatus === "aprovado" ? valorFinal : null,
    })
    .eq("id", id)
    .select()
    .single()
      ...
      try {
        await fetch(webhookUrl, {
          ...
          body: JSON.stringify({
            ...
            mes_ano: updated.mes_ano,
            meta_aprovada: valorFinal,
            aprovado_por: email,
            comentario_revisor: parsed.data.comentario ?? null,
          }),
        })
      } catch (err) {
        console.error("[metas-ajustes] webhook trigger failed:", err)
        await supabase
          .from("metas_ajustes_propostos")
