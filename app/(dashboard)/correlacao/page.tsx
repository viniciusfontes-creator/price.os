...
type ChartMode = 'checkin' | 'extraction' | 'comparativo_interno';
...
export default function MarketMonitorPage() {
    ...
    const { data: basketResponse, isLoading: basketsLoading, isValidating: basketsValidating, mutate: mutateBaskets } = useSWR<{ success: boolean, data: Basket[] }>(
        `/api/baskets?withHistory=true&${queryParams}`,
        fetcher,
        {
    ...
    const chartData = useMemo(() => {
        ...
        [selectedBasket].forEach(basket => {
            basket.basket_items?.forEach(item => {
                const itemKey = item.item_type === 'internal' ? (item.internal_property_data?.idpropriedade || item.internal_property_id) : (item.airbnb_listing_id || item.id);
                ...
                item.history?.forEach((h: any) => {
                    ...
                    const prefix = item.item_type === 'internal' ? 'internal_' : 'listing_';

                    if (chartMode === 'checkin' || chartMode === 'comparativo_interno') {
                        ...
                        if (!currentStored || new Date(currentStored.extraction) < extractionDate) {
                            dataPoints[xAxisKey][`${prefix}${itemKey}`] = value;
                        ...
                        dataPoints[xAxisKey][`acc_prefix_${itemKey}`] = prefix;
                    }
                });
            });
        });
        ...
        if (chartMode === 'extraction') {
            Object.keys(dataPoints).forEach(key => {
                ...
                Object.keys(point).forEach(prop => {
                    if (prop.startsWith('acc_') && !prop.startsWith('acc_prefix_')) {
                        const itemKey = prop.replace('acc_', '');
                        const { sum, count } = point[prop];
                        const prefix = point[`acc_prefix_${itemKey}`] || 'listing_';
                        point[`${prefix}${itemKey}`] = parseFloat((sum / count).toFixed(2));
                    }
                });
            });
        }
    ...
    const richChartData = useMemo(() => {
        return chartData.map(point => {
            ...
            const internalValues: number[] = [];
            Object.keys(point).forEach(k => {
                if (k.startsWith('listing_')) externalValues.push(point[k]);
                if (k.startsWith('internal_')) internalValues.push(point[k]);
            });
            const avg = externalValues.length > 0
                ? externalValues.reduce((a, b) => a + b, 0) / externalValues.length
                : 0;

            const intAvg = internalValues.length > 0
                ? internalValues.reduce((a, b) => a + b, 0) / internalValues.length
                : undefined;
            ...
            return {
                ...point,
                avgPrice: avg,
                ...(intAvg !== undefined ? { internalAvgPrice: intAvg } : {})
            };
        });
    }, [chartData]);
    ...
    const handlePointClick = (payload: any) => {
        ...
        const internalAvgPrice = payload.internalAvgPrice;
                    ...
                    item.history?.forEach((h: any) => {
                        ...
                        if (hCheckin === dateStr) {
                            ...
                            if (extraction !== 'Unknown') {
                                ...
                                const isInternal = item.item_type === 'internal';
                                const itemId = isInternal ? (item.internal_property_data?.idpropriedade || item.internal_property_id) : (item.airbnb_listing_id || item.id);
                                ...
                                if (!isDuplicate) {
                                    ...
                                    if (val) {
                                        ...
                                        historyMap[extraction].items.push({
                                            id: itemId,
                                            name: isInternal ? (item.internal_property_data?.nome || `Imóvel Interno ${itemId}`) : (item.airbnb_data?.nome_anuncio || `Anúncio ${itemId}`),
                                            price: val,
                                            isInternal
                                        });
                                    }
        ...
        const history = Object.keys(historyMap).map(d => ({
            date: d,
            value: historyMap[d].count > 0 ? historyMap[d].sum / historyMap[d].count : 0,
        ...
        const items = Object.keys(payload)
            .filter(k => k.startsWith('listing_') || k.startsWith('internal_'))
            .map(k => {
                const isInternal = k.startsWith('internal_');
                const id = k.replace(isInternal ? 'internal_' : 'listing_', '');
                // Find full data
                let fullData = null;
                let propName = '';
                let displayId = id;
                for (const b of activeBaskets) {
                    const found = b.basket_items?.find((i: BasketItem) =>
                        isInternal
                            ? (i.internal_property_id?.toString() === id.toString() || i.internal_property_data?.idpropriedade?.toString() === id.toString())
                            : (i.airbnb_listing_id || i.id).toString() === id.toString()
                    );
                    if (found) {
                        fullData = found;
                        if (isInternal) {
                            propName = fullData.internal_property_data?.nome || `Imóvel Interno ${id}`;
                            displayId = fullData.internal_property_data?.idpropriedade || id;
                        } else {
                            propName = competitorMap[id] || fullData.airbnb_data?.nome_anuncio || `Anúncio ${id}`;
                        }
                        break;
                    }
                }
                if (!propName) propName = isInternal ? `Imóvel Interno ${id}` : `Anúncio ${id}`;

                return {
                    id: displayId,
                    internalUuid: isInternal ? id : undefined,
                    name: propName,
                    price: payload[k],
                    fullData,
                    isInternal
                        ...
                        {selectedBasket ? (
                            <>
                                        ...
                                        <div className="flex flex-wrap gap-4 items-center w-full lg:w-auto">
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <div className="flex bg-muted p-1 rounded-lg">
                                                    ...
                                                    <button
                                                        onClick={() => setChartMode('comparativo_interno')}
                                                        className={cn(
                                                            "px-3 py-1.5 text-xs font-bold rounded-md transition-all",
                                                            chartMode === 'comparativo_interno' ? "bg-background shadow-sm text-foreground text-emerald-600" : "text-muted-foreground hover:text-foreground hover:text-emerald-600"
                                                        )}
                                                    >
                                                        Comparativo Interno
                                                    </button>
                                ...
                                <DailyCompositionDialog
                                    ...
                                    internalAvgPrice={compositionDialog.internalAvgPrice}
